    ...
    def connect(
        ...
        **kwargs: ConnParam,
        ...
        if not yb_params.smart_driver_enabled:
            yb_logger.debug(
                "smart driver disabled (load_balance_hosts absent or pass-through); calling upstream _aconnect_plain"
            )
        ...
        # --- Smart-driver path ---
        yb_logger.debug(
            "smart driver enabled (topology_keys=%s, refresh_interval=%ds, ttl=%ds, xcluster=%s)",
        ...
        if yb_params.xcluster_enabled:
            ...
            if which is None:
                raise NoViableClusterError(
                    f"xCluster: both primary and secondary clusters report UNHEALTHY — no viable target for new connections (primary_uuid={failover_group.primary.uuid})"
                )
            if which == "secondary":
        ...
        while True:
            ...
            if node is None:
                ...
                yb_logger.warning(
                    "%s (attempted=%s, last_err=%s)", msg, sorted(attempted), last_err
                )
            ...
            yb_logger.debug(
                ...
                node.host,
                state.uuid,
                active_cluster or "(single-cluster)",
            )
            return conn
